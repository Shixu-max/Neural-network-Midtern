...
def coords_fmap2orig(feature,stride):
    '''
    feature map coordinate -> original coordinate
    '''
...
class GenTargets(nn.Module):
    ...
    def _gen_level_targets(self,out,gt_boxes,classes,stride,limit_range,sample_radiu_ratio=1.5):
        ...
        cnt_targets=((left_right_min*top_bottom_min)/
                     (left_right_max*top_bottom_max+1e-10)).sqrt().unsqueeze(dim=-1)#[batch_size,h*w,1]
...
def compute_cls_loss(preds,targets,mask):
    ...
    for batch_index in range(batch_size):
        ...
        target_pos=(torch.arange(1,class_num+1,device=target_pos.device)[None,:]==target_pos).float()#onehot
