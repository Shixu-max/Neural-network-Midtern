...
if __name__=="__main__":
    ...
    model.load_state_dict({k.replace('module.',''):v for k,v in torch.load("./checkpoint/fcos-25.pth",map_location=torch.device('cpu')).items()})
    ...
    for name in names:
        ...
        plt.savefig('img_out/{}'.format(name), bbox_inches='tight', pad_inches=0.0)
        plt.close()