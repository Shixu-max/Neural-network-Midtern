...
from .val_eval import eval_forward
...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    mlosses = {"loss_classifier": torch.zeros(1).to(device), "loss_box_reg": torch.zeros(1).to(device),
               "loss_objectness": torch.zeros(1).to(device), "loss_rpn_box_reg": torch.zeros(1).to(device)}
